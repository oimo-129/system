...
function setab(name, cursel, n, link) {
  ...
  if (link != null && link != "")
    document.getElementById("TabLink" + name).href = link;
}
...
function ScrollPic(scrollContId, arrLeftId, arrRightId, dotListId) {
    ...
    if (this.arrLeftId) {
      ...
      if (this.arrLeftObj) {
        ...
        zhangxu.addEvent(
          this.arrLeftObj,
          "mouseup",
          Function("ScrollPic.childs[" + this.ID + "].rightEnd()")
        );
        zhangxu.addEvent(
          this.arrLeftObj,
          "mouseout",
    ...
    if (this.dotListId) {
      ...
      if (this.dotListObj) {
        var pages = Math.round(
            this.listDiv01.offsetWidth / this.frameWidth + 0.4
          ),
          i,
          tempObj;
        for (i = 0; i < pages; i++) {
          ...
          zhangxu.addEvent(
            tempObj,
            "click",
            Function("ScrollPic.childs[" + this.ID + "].pageTo(" + i + ")")
          );
        }
      }
    }
  ...
  this.leftMouseDown = function () {
    ...
    _scrollTimeObj = setInterval(
      "ScrollPic.childs[" + this.ID + "].moveLeft()",
      this.speed
    );
  };
  ...
  this.leftEnd = function () {
    ...
    var fill =
      this.pageWidth - (this.scrollContDiv.scrollLeft % this.pageWidth);
    this.move(fill);
  };
  ...
  this.move = function (num, quick) {
    ...
    if (thisMove > 0) {
      ...
      if (this.scrollContDiv.scrollLeft - thisMove <= 0) {
        this.scrollContDiv.scrollLeft =
          this.listDiv01.scrollWidth + this.scrollContDiv.scrollLeft - thisMove;
      } else {
        this.scrollContDiv.scrollLeft += thisMove;
      }
    }
    ...
    if (Math.abs(num) == 0) {
      ...
      this.accountPageIndex();
      setTimeout(
        "ScrollPic.childs[" + this.ID + "].move(" + num + "," + quick + ")",
        this.speed
      );
    }
  };
...
function Marquee() {
  ...
  this.ID.style.overflow =
    this.ID.style.overflowX =
    this.ID.style.overflowY =
      "hidden";
  this.ID.noWrap = true;
...
Marquee.prototype.Start = function () {
  ...
  if (typeof this.Direction == "string")
    this.Direction =
      this.DirectionArray[this.Direction.toString().toLowerCase()];
  ...
  if (msobj.Direction <= 1) {
    msobj.ID.innerHTML = templateTop.replace(
      /MSCLASS_TEMP_HTML/g,
      msobj.ID.innerHTML
    );
  } else {
    if (msobj.ScrollStep == 0 && msobj.DelayTime == 0) {
      ...
      msobj.ID.innerHTML = templateLeft.replace(
        /MSCLASS_TEMP_HTML/g,
        msobj.ID.innerHTML
      );
    }
  ...
  msobj.Begin = function () {
    ...
    if (
      (msobj.Direction <= 1 &&
        msobj.ClientScroll <= msobj.Height + msobj.Step) ||
      (msobj.Direction > 1 && msobj.ClientScroll <= msobj.Width + msobj.Step)
    ) {
    ...
    msobj.ID.onmousemove = function (event) {
      if (msobj.ScrollStep == 0 && msobj.Direction > 1) {
        ...
        if (window.event) {
          if (msobj.IsNotOpera) {
            msobj.EventLeft =
              event.srcElement.id == msobj.ID.id
        ...
        msobj.Step = Math.round(
          (msobj.AbsCenter * (msobj.BakStep * 2)) / msobj.HalfWidth
        );
      }
    };
